...
class Graph{
    ...
    public:
        ...
        void DFS ( int start );  // prints DFS traversal of the complete graph
};
...
void Graph::DFS ( int start ){ // DFS Traverse of the vertices reachable from start
    ...
    while ( !st.empty() ) {
        ...
        for ( it = AdjMat[ top ].begin(); it != AdjMat[ top ].end(); it++ ) {
            if ( !visited[ *it ] )
                st.push( *it );
        }
    }
...
void Test_Input() {
    ...
    printf ("Following is Depth First Traversal (startting from vertices 2) \n");
    g.DFS ( 2 );
}
